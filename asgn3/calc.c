...
int main(int argc, char **argv) {
	...
	fprintf(stderr, "> ");
        if (fgets(input_line, STACK_CAPACITY, stdin) == NULL) {
		break;
	}
	...
	while (token != NULL && !error) {
	    if (parse_double(token, &x)) {
		    ...
		    error = true;
            } else if (binary_operators[(unsigned)*token] != 0 && binary_operators[(unsigned)*token] != NULL) {
	    ...
	    } else if (unary_operators[(unsigned)*token] != 0 && unary_operators[(unsigned)*token] != NULL) {
